...
def _safe_question_alignment_stats(response_hidden: np.ndarray, question_hidden: np.ndarray) -> dict[str, float]:
    if response_hidden.size == 0 or question_hidden.size == 0:
        return {
            "q_align_maxcos_mean": 0.0,
            "q_align_maxcos_std": 0.0,
            "q_align_maxcos_min": 0.0,
            "q_align_maxcos_p10": 0.0,
            "q_align_low_support_rate": 0.0,
        }

    response_norm = response_hidden / np.clip(np.linalg.norm(response_hidden, axis=1, keepdims=True), 1e-12, None)
    question_norm = question_hidden / np.clip(np.linalg.norm(question_hidden, axis=1, keepdims=True), 1e-12, None)
    sim = response_norm @ question_norm.T
    max_per_response = sim.max(axis=1)
    return {
        "q_align_maxcos_mean": float(max_per_response.mean()),
        "q_align_maxcos_std": float(max_per_response.std()),
        "q_align_maxcos_min": float(max_per_response.min()),
        "q_align_maxcos_p10": _safe_percentile(max_per_response, 10),
        "q_align_low_support_rate": float((max_per_response < 0.3).mean()),
    }
...
def extract_features_one(
    ...
    question_hidden = hidden_states[-1][0, :min(50, resp_start), :].float().cpu().numpy()
    features.update(_safe_question_alignment_stats(last_hidden, question_hidden))
    return features
