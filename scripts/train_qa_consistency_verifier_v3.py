...
from qa_consistency_verifier_v3 import build_feature_name_list, fit_text_vectorizers, transform_text_features


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train no-retrieval QA consistency verifier v3")
    parser.add_argument(
        "--labels-paths",
        type=str,
        default="artifacts_poc/internal_real_v1.parquet,artifacts_poc/internal_factual_v1.parquet",
    )
    parser.add_argument("--artifacts-dir", type=str, default="detector_artifacts_qa_consistency_v3")
...
def main() -> None:
    ...
    bundle = {"model": clf, "vectorizers": vectorizers}
    joblib.dump(bundle, artifacts_dir / "qa_consistency_verifier.joblib")
    feature_names = build_feature_name_list(vectorizers, dense_train.columns.tolist())
    (artifacts_dir / "feature_columns.json").write_text(json.dumps(feature_names, ensure_ascii=False, indent=2), encoding="utf-8")
    (artifacts_dir / "metrics.json").write_text(json.dumps({
        ...
        "version": "qa_consistency_v3_first_span",
    ...
    valid_out.to_parquet(artifacts_dir / "validation_predictions.parquet", index=False)
    dense_valid.to_parquet(artifacts_dir / "validation_dense_features.parquet", index=False)

    print(f"saved model: {artifacts_dir / 'qa_consistency_verifier.joblib'}")
